...
SELECT TO_CHAR(sale_date, 'YYYY-MM-DD') AS sale_date, MAX(diff) KEEP (DENSE_RANK FIRST ORDER BY fruit, sale_date) AS diff
FROM(
...
-- Fix: Write the correct format for the date
SELECT TO_CHAR(a.sale_date, 'YYYY-MM-DD') AS sale_date, a.sold_num-o.sold_num AS diff
