...
SELECT time.activity_date AS day,
COUNT(DISTINCT a.user_id) AS active_users FROM
(SELECT DISTINCT activity_date FROM Activity
WHERE activity_date BETWEEN TO_DATE('2019-06-28','YYYY-MM-DD') AND TO_DATE('2019-07-27','YYYY-MM-DD')) time
JOIN Activity a ON time.activity_date = a.activity_date
GROUP BY time.activity_date
ORDER BY time.activity_date;
