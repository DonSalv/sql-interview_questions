...
ROUND(AVG(ride_duration) OVER (ORDER BY month ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING),2) AS average_ride_duration FROM
(SELECT Months.month, COALESCE(q1.ride_distance,0) AS ride_distance, 
COALESCE(q1.ride_duration,0) AS ride_duration FROM
(SELECT EXTRACT(MONTH FROM month) AS month FROM
...
SUM(ar.ride_distance) AS ride_distance, SUM(ar.ride_duration) AS ride_duration
FROM AcceptedRides ar JOIN Rides r ON r.ride_id = ar.ride_id
WHERE EXTRACT(YEAR FROM requested_at)=2020
GROUP BY EXTRACT(MONTH FROM r.requested_at)) Q1
ON months.month=Q1.month
